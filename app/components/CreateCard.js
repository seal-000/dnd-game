...
import { storage, db } from '@/app/utils/firebase/config'; 
...
function CreateCard() {
  ...
  const createCharacter = async (event) => {
    ...
    const imageRef = ref(storage, `dndCharacters/${character.name}.jpg`); // Set a unique name for the image

    try {
      await uploadString(imageRef, image, 'data_url');
      const characterData = { ...character, image }; // Add image URL
      await addDoc(collection(db, 'character'), characterData); // Add character data to Firestore
      alert("Success! You created a character!");
      router.push('/gallery'); // Navigate to your target route here
    } catch (error) {
        ...
        <div className='form-container'>
          ...
          <FormControl component="fieldset">
            ...
            <RadioGroup
              aria-label="class"
              name="class"
              value={character.class} // Bind this to the character class state
